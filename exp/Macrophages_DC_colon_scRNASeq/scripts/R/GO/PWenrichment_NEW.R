...
pwe <- compareCluster(Entrez~group, data=df, fun="enrichPathway", organism=orgname, pvalueCutoff = pval, pAdjustMethod = "BH", qvalueCutoff = qval, readable = TRUE)
...
if (length(pwe@compareClusterResult$Description) > 0) {
	...
	pdf(PLOT, width=15, height=height)
